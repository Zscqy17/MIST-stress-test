...
class MISTApp:
    def __init__(self, root):
        ...
        self.TIME_LIMIT = 8.0          # 每题限时(秒)，制造压力
        self.INTERMISSION_TIME = 5     # 轮间休息倒计时(秒)
        
        # --- 单词记忆配置 ---
        self.WORD_DISPLAY_TIME = 2000 # 毫秒
        
        # 单词库 (中文, 英文) - 总共需要 3轮 * 10个 = 30个目标词，以及 30个干扰词
        self.all_targets = [
        ...
        self.style.configure("TLabel", font=("Helvetica", 18), background="#f0f0f0")
        self.style.configure("Header.TLabel", font=("Helvetica", 32, "bold"), background="#f0f0f0")
    ...
    def show_login_screen(self):
        ...
        tk.Label(self.main_frame, text="Montreal Image Stress Test", font=("Helvetica", 40, "bold"), bg="#f0f0f0").pack(pady=60)
        
        input_frame = tk.Frame(self.main_frame, bg="#f0f0f0")
        input_frame.pack(pady=30)
        
        tk.Label(input_frame, text="请输入被试 ID:", font=("Helvetica", 24), bg="#f0f0f0").pack(side="left", padx=15)
    ...
    def show_intermission_screen(self, first_start=False):
        ...
        title_text = f"准备开始第 {self.current_round} 轮" if not first_start else "测试准备就绪"
        tk.Label(self.main_frame, text=title_text, font=("Helvetica", 36, "bold"), bg="#f0f0f0").pack(pady=60)
        ...
        tk.Label(self.main_frame, text=info_text, font=("Helvetica", 24), bg="#f0f0f0", justify="center").pack(pady=30)
        
        # 手动开始提示
        tk.Label(self.main_frame, text="请点击下方按钮开始本轮测试", font=("Helvetica", 20), fg="gray", bg="#f0f0f0").pack(pady=40)
    ...
    def next_trial_step(self):
        # 检查是否完成本轮所有题目
        if self.current_question_index >= self.QUESTIONS_PER_ROUND:
    ...
    def setup_question_ui(self):
        ...
        tk.Label(top_frame, text=f"第 {self.current_round} / {self.TOTAL_ROUNDS} 轮", font=("Helvetica", 20), bg="#f0f0f0").pack(side="left")
        tk.Label(top_frame, text=f"题目: {self.current_question_index} / {self.QUESTIONS_PER_ROUND}", font=("Helvetica", 20), bg="#f0f0f0").pack(side="right")
        
        # 算术题显示
        tk.Label(self.main_frame, text=self.current_question_data["expression"] + " = ?", font=("Helvetica", 72, "bold"), bg="#f0f0f0").pack(pady=50)
        ...
        self.feedback_label = tk.Label(self.main_frame, text="", font=("Helvetica", 24), bg="#f0f0f0")
    ...
    def update_timer(self):
        ...
        percentage = (self.remaining_time / self.TIME_LIMIT) * 100
    ...
    def handle_timeout(self):
        self.timer_running = False
        self.record_result(None, False, self.TIME_LIMIT, timeout=True)
    ...
    def show_word_display(self):
        ...
        tk.Label(self.main_frame, text="请记忆", font=("Helvetica", 24), fg="gray", bg="#f0f0f0").pack(pady=40)
        
        tk.Label(self.main_frame, text=zh_word, font=("Helvetica", 80, "bold"), fg="#2980b9", bg="#f0f0f0").pack(pady=20)
        tk.Label(self.main_frame, text=en_word, font=("Helvetica", 40), fg="#34495e", bg="#f0f0f0").pack(pady=10)
    ...
    def show_recall_screen(self):
        ...
        tk.Label(self.main_frame, text=f"第 {self.current_round} 轮 - 单词回忆", font=("Helvetica", 40, "bold"), bg="#f0f0f0").pack(pady=20)
        tk.Label(self.main_frame, text="请勾选刚才出现过的所有单词", font=("Helvetica", 24), bg="#f0f0f0").pack(pady=10)
        ...
        for i, word in enumerate(all_options):
            ...
            cb = tk.Checkbutton(options_frame, text=word, variable=var, font=("Helvetica", 18), bg="#f0f0f0")
    ...
    def show_results_screen(self):
        ...
        tk.Label(self.main_frame, text="测试完成", font=("Helvetica", 40, "bold"), bg="#f0f0f0").pack(pady=40)
        ...
        tk.Label(self.main_frame, text=f"结果已保存至 results/mist_results_{self.subject_id}.csv", font=("Helvetica", 16), fg="gray", bg="#f0f0f0").pack(pady=30)
